...
for (const p of productos) {
  ...
  if (!["unas", "herramientas", "pestanas", "facial", "cuerpo"].includes(p.categoria)) {
...
console.log(`categorías     ${["unas", "herramientas", "pestanas", "facial", "cuerpo"].map((c) => `${c} ${productos.filter((p) => p.categoria === c).length}`).join(" · ")}`);
console.log();
